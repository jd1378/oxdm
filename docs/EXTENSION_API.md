...
| `filename`    | string                | no       | Suggested filename. oxdm overrides with the server-provided name when present, and numbers it (`file_1.zip`) if another download in the list already has that name — names identify downloads, so they are unique across the whole list regardless of folder. |
