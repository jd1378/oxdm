    ...
    P::Theme: theme::Base,
{
    ...
    'next_event: loop {
        ...
        // `try_recv` errs on both empty and closed; `next().await` then
        // resolves the difference — pending work or `None` for closed.
    ...
    match action {
        ...
        Action::Window(action) => match action {
            ...
            window::Action::Resize(id, size) => {
                if let Some(window) = window_manager.get_mut(id) {
                    let requested = winit::dpi::LogicalSize {
                        width: size.width,
                        height: size.height,
                    }
                    ...
                    // oxdm fix: on Wayland (and any backend that resizes
                    // synchronously) winit applies the new size itself and
                    // returns it *instead of* emitting `Resized`, so the
                    // viewport, the surface and the layout would keep the
                    // pre-resize dimensions forever — content stays laid
                    // out and blitted at the old size while the compositor
                    // shows the new one. Feed the applied size back through
                    // the same path the winit event would have taken.
                    if let Some(new_size) =
                        window.raw.request_inner_size(requested)
                    {
                        window.state.update(
                            program,
                            &window.raw,
